...
namespace Recognition.Application
{
    public partial class NetworkForm : Form
    {
        ...
        public bool RecognizeImage(Bitmap input, string target, Figure figure)
        {
            Bitmap bitmap = new Bitmap(input, new Size(Network.InputImageWidth, Network.InputImageHeight));
            bitmap.RotateFlip(RotateFlipType.Rotate90FlipNone);

            NormalizedImage image = new NormalizedImage(bitmap);
            ...
            if (maxValueIndex.ToString() == target) return true;
            return false;
        }
